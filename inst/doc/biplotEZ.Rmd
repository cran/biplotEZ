...
If the user wishes to view the variables as arrows on the biplot to give information on the adequecy of the variables, this can be done with the `axes()` function, by setting `vectors = TRUE` and `unit.circle = TRUE`. The adequacy value is given by squared length of the arrow. 
